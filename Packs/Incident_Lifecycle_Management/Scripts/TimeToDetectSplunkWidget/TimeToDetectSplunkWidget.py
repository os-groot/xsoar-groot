...
register_module_line('TimeToDetectSplunkWidget', 'start', __line__())
'''IMPORTS'''
...
FIELDS = {'Incident Created': 'created', 'Incident ID': 'id', 'Incident Name': 'name',
          'Detected Time_Sec.Device': 'CustomFields.detectedtime', 'Source': 'sourceInstance',
          'Severity': 'severity', 'Splunk Search Time': 'CustomFields.splunksearchtime'
          }
DISPLAYED_TIME_FIELDS = ['Incident Created', 'Detected Time_Sec.Device', 'Splunk Search Time']
QUERY_SIZE = 5000
SORT_FIELD = 'created'
...
def readable_time(a_time):
    if a_time is None:
        return 'Empty'
    if isinstance(a_time, dtime.datetime):  # Stop Process if received a Date Time Object
        return a_time
...
def set_columns(incident, fields_map=None):
    ...
    created_time = demisto.get(incident, 'created')  # Creation date of incident in XSOAR
    created_time = str_date_parser(created_time, return_date_time=True)  # Creation date of incident in XSOAR
    ...
    if search_time is None or str(search_time) == '' or str(search_time) == str(None):
        if info_search_time != '':
            ...
            search_time = created_time
    else:
        display_inc['Splunk Search Time'] = readable_time(search_time)
        search_time = str_date_parser(search_time, to_tz=TZ, return_date_time=True)
    # Get Detected time
    detected_time = inc_custom.get('detectedtime')
    if detected_time is None or str(detected_time) == '' or str(detected_time) == str(None):
        if search_time == created_time:
            detected_time = created_time
            display_inc['Detected Time_Sec.Device'] = f'XSOAR Created Time: {readable_time(detected_time)}'
        else:
            detected_time = search_time
            display_inc['Detected Time_Sec.Device'] = f'Splunk Search Time: {readable_time(search_time)}'
    detected_time = str_date_parser(detected_time, to_tz=TZ, return_date_time=True)
    '''Time to escalation is computed as TTD = ("Creation Date" - "Security Control Time(detectedtime)")'''
    time_diff = created_time - detected_time
    ...
    display_inc['Time Consumed To Detect (Minutes)'] = int(time_diff_minutes)
...
if __name__ in ('__main__', '__builtin__', 'builtins'):
    main()
register_module_line('TimeToDetectSplunkWidget', 'end', __line__())
